...
// net.Server with the closeAllConnections shim attached by startServer (and by the
// inline servers of tests that need byte-level control), see safeDone
type TestServer = net.Server & { closeAllConnections?: () => void };
...
describe('SMTP-Connection Tests', () => {
    ...
    function startServer(replies: { [command: string]: Buffer | string }, callback: (server: TestServer) => void) {
        let openSockets = new Set<net.Socket>();
        let server: TestServer = net.createServer(socket => {
    ...
    function toBuf(payload: Buffer | string): Buffer {
        return Buffer.isBuffer(payload) ? payload : Buffer.from(payload, 'utf8');
    }

    function makeClient(server: TestServer) {
    ...
    function safeDone(server: TestServer, done: (err?: any) => void, fn: () => void) {
    ...
    describe('close() during DNS resolution', () => {
        it('does not open a socket when closed while DNS is in flight', (t, done) => {
            ...
            server.listen(0, '127.0.0.1', () => {
                ...
                // shared.resolveHostname is a frozen ES module export, so the deferral
                // happens one level down, at the dns.Resolver it creates: the A lookup
                // is held back until deferred() runs and the AAAA lookup answers right
                // away, so the resolver callback lands in _resolveAndConnect only once
                // deferred() has been called.
                shared.dnsCache.delete('deferred.invalid');
                t.mock.method(
                    dns.Resolver.prototype,
                    'resolve4',
                    (hostname: string, cb: (err: Error | null, addresses: string[]) => void) => {
                        deferred = () => cb(null, ['127.0.0.1']);
                    }
                );
                t.mock.method(
                    dns.Resolver.prototype,
                ...
                setImmediate(() => {
                    ...
                    // restore immediately once the deferred lookup has been answered, before
                    // any other test runs. The AAAA lookup runs inside deferred(), so the
                    // resolver must still be mocked at that point
                    t.mock.restoreAll();
                    setTimeout(() => {
                        server.close(() => {
                            let assertErr = null;
    ...
    describe('Server response decoding', () => {
        it('should decode UTF-8 in error response and propagate via err.response and err.message', (t, done) => {
            ...
            startServer(
                ...
                server => {
                    ...
                    client.connect(() => {
                        client.send({ from: 'a@example.com', to: ['b@example.com'] }, 'test', (err: any) => {
                            safeDone(server, done, () => {
                                assert.ok(err);
        ...
        it('should decode UTF-8 in success response and propagate via info.response', (t, done) => {
            ...
            startServer(
                ...
                server => {
                    ...
                    client.connect(() => {
                        client.send({ from: 'a@example.com', to: ['b@example.com'] }, 'test message', (err: any, info: any) => {
                            safeDone(server, done, () => {
        ...
        it('should decode multi-line UTF-8 EHLO response and still parse extensions', (t, done) => {
            ...
            startServer({ ehlo }, server => {
                ...
                client.connect(() => {
                    let lastResp = client.lastServerResponse as string;
        ...
        it('should leave pure ASCII responses unchanged', (t, done) => {
            ...
            startServer({ dataDone: '250 2.0.0 OK queued as ABC123\r\n' }, server => {
                ...
                client.connect(() => {
                    client.send({ from: 'a@example.com', to: ['b@example.com'] }, 'plain ascii', (err: any, info: any) => {
                        safeDone(server, done, () => {
                            assert.ok(!err);
    ...
    describe('Connection tests', () => {
        let server: any, insecureServer: any, invalidServer: net.Server, secureServer: any, disconnectingServer: any, httpProxy: any;
        ...
        before((t, done) => {
            ...
            disconnectingServer = new SMTPServer({
                disabledCommands: ['STARTTLS', 'AUTH'],
                onConnect(this: any, sess: any, callback: any) {
                    setTimeout(() => {
        ...
        it('should close connection after STARTTLS', (t, done) => {
            ...
            client.connect(() => {
                assert.strictEqual(client.secure, true);
                server.connections.forEach((conn: any) => {
                    conn.close();
                });
            });
        ...
        it('should connect through proxy', (t, done) => {
            let runTest = function (socket: net.Socket) {
                let client = new SMTPConnection({
                    logger: false,
                    port: PORT_NUMBER,
                    connection: socket
    ...
    describe('Login tests', () => {
        ...
        beforeEach((t, done) => {
            ...
            lmtpServer = new SMTPServer({
                ...
                onData: (stream: any, session: any, callback: any) => {
                    stream.on('data', () => {});
                    stream.on('end', () => {
                        let response = session.envelope.rcptTo.map((rcpt: any, i: number) => {
                            if (i % 2) {
        ...
        describe('xoauth2 login', () => {
            ...
            it('should fail with invalid xoauth2 response', (t, done) => {
                ...
                t.mock.method(oauth2, 'generateToken', (cb: any) => cb(null, 'dXNlcj10ZXN0dXNlcgFhdXRoPUJlYXJlciB1dXV1dQEB'));
        ...
        describe('custom login', () => {
            let customClient: SMTPConnection;
            beforeEach((t, done) => {
    ...
    let socket = net.connect(port, host, () => {
        ...
        let onSocketData = (chunk: Buffer) => {
            ...
            if ((match = headers.match(/\r\n\r\n/))) {
                ...
                remainder = headers.substr((match.index as number) + match[0].length);
